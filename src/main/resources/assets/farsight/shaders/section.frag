...
vec3 hashColor(uint id) {
    // Quick hash-to-color so blockstates are visually distinguishable before
    // the texture atlas is wired up. Golden-ratio scrambler.
...
void main() {
    ...
    vec3 lit = base * faceLight(v_faceIdx);
